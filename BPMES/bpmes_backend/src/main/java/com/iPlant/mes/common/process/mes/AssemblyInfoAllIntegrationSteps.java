...
public class AssemblyInfoAllIntegrationSteps implements TemplateMethodHandleSteps {
	...
	public int Steps(DataExchangeAssembly dataExchange, CommonResult result) {
		...
		String CARE_LB =(String)parameter.get("CARE_LB");  //组装传入载具条码
		...
		if(!CARE_LB.equals("") || CARE_LB !=null){
			
			Map   careSnMap = new HashMap();
			careSnMap.put("CARE_LB", CARE_LB);
			careSnMap.put("BAR_CODE", BAR_CODE);
			returnNum = CoreFrameDao.doEngineCall("queryCarrierBinding", careSnMap, result);
			if (returnNum != CoreFrameErrorCode.ISUCCESS_CODE) {
				throw new PodiumException(returnNum, result.getPrompt(),
						CoreFrameworkConstants.Podium_LVL);
			}
			...
			returnNum = CoreFrameDao.doEngineCall("insertCarrierBinding", careSnMap, result);
			if (returnNum != CoreFrameErrorCode.ISUCCESS_CODE) {
				throw new PodiumException(returnNum, result.getPrompt(),
						CoreFrameworkConstants.Podium_LVL);
			}
			returnNum = CoreFrameDao.doEngineCall("insertCarrierBindingDetailed", careSnMap, result);
			if (returnNum != CoreFrameErrorCode.ISUCCESS_CODE) {
				throw new PodiumException(returnNum, result.getPrompt(),
						CoreFrameworkConstants.Podium_LVL);
			}
		...
		/*产品装配   START*/
		Map<String,String> mapAssembly =new HashMap<String,String>();
		mapAssembly.put("BAR_CODE", BAR_CODE);
		mapAssembly.put("ROUT_CD", ROUT_CD);
		mapAssembly.put("MAT_CD", MAT_CD);
		mapAssembly.put("MAT_NM", MAT_NM);
		returnNum = CoreFrameDao.doEngineCall("querySnWorkOrderInformation", mapAssembly, result);
		if (returnNum != CoreFrameErrorCode.ISUCCESS_CODE) {
			throw new PodiumException(returnNum, result.getPrompt(),
					CoreFrameworkConstants.Podium_LVL);
		}
		if (result.getDataList() != null && result.getDataList().size() > 0) {
			for (Iterator<Map> iter = result.getDataList().iterator(); iter.hasNext();) {
				...
				mapAssembly.put("FCT_CD", (String)param.get("FCT_CD"));           //工厂编码
				mapAssembly.put("WC_CD", (String)param.get("WC_CD"));		      //车间编码
				mapAssembly.put("BAR_CODE", (String)param.get("BAR_CODE"));       //产品条码
				mapAssembly.put("WO_NO", (String)param.get("WO_NO"));             //作业指示
				mapAssembly.put("MO_NO", (String)param.get("MO_NO"));             //工单编号
				mapAssembly.put("PRNT_MAT_CD", (String)param.get("ITEM_CD"));         //产品编号
				mapAssembly.put("PRNT_MAT_NM", (String)param.get("ITEM_NM"));         //产品名称
				mapAssembly.put("PRF_CD", (String)param.get("PRF_CD"));           //工序代码
			}
		}
		
		/*装配*/
		returnNum=CoreFrameDao.doEngineCall("insertAssemblyInformation",mapAssembly,result);	    
		if (returnNum != CoreFrameErrorCode.ISUCCESS_CODE) {
			throw new PodiumException(returnNum, result.getPrompt(),
			  CoreFrameworkConstants.Podium_LVL);
		}
