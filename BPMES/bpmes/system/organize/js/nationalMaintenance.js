...
(function() {
	function nationalMaintenance() {
		...
		initGridData = function() {
			...
			reqGridData('/iPlant_ajax', 'national_tab', reqData);
		},
		bindGridData = function(reqData, jsonData) {
			var gridList = {
				...
				columns: [[
					...
					{field: 'CY_CD',title: '国家代码',width: 100,align: 'center',formatter: function (value) {if(checkNotEmpty(value)) return "<span title='" + value + "'>" + value + "</span>";},editor:{type:'validatebox',
			        	   options:{required:true, validType:['length[1,15]','specialTextCharacter']}}},
			        {field: 'CY',title: '国家',width: 100,align: 'center',formatter: function (value) {if(checkNotEmpty(value)) return "<span title='" + value + "'>" + value + "</span>";},editor:{type:'validatebox',
...
//			    	 row.DICT_IT_NM = $(eddi.target).combobox('getText');
		        ...
		        onClickRow: function (index, row) {
			    	if (editIndex != index){
			    		...
			    		if(editIndex!=undefined){
			    			...
			    			var rowEdit = dataGrid.datagrid('getRows')[editIndex],edft = $(this).datagrid('getEditor', {index: editIndex,field: 'CY_CD'}),editorFt = edft.target,ftCd = editorFt.val();
			    			if(checkNotEmpty(rowEdit.editType)){
			    				if(rowEdit.editType=='add'){
			    					...
			    					if(!checkNotEmpty(row.editType)){//如果是修改的情况，ft_cd字段为只读模式
							    		ed = $(this).datagrid('getEditor', {index: index,field: 'CY_CD'});
							    		fc = ed.target;
		...
		conditionSearch = function(){
			var nationalName = $('#nationalCode').textbox('getValue');
			var dgrid = dataGrid.datagrid('options');
			if(!dgrid) return;
			var reqData = {
				IFS: 'B000089',
            	...
            	if (dataGrid.datagrid('getChanges').length) {
                    ...
                    if(inserted.length>0){
                    	for(var m=0;m<inserted.length;m++){
                    		var date = new Date();
                    		var syeTime = dateToString(date);
                    		arrInsert.push({ CD: 'CY'+syeTime+m,CY_CD: inserted[m].CY_CD,CY: inserted[m].CY,CRT_ID: inserted[m].CRT_ID,CRT_DT: inserted[m].CRT_DT});
                    	}
                    	//批量先增
                        var ajaxInsert = {
                            url: '/iPlant_ajax',
                    ...
                    if(updated.length>0){
                    	for(var m=0;m<updated.length;m++){
                    		if(updated[m].edited){
                    			arrUpdate.push({ CD: updated[m].CD,CY_CD: updated[m].CY_CD,CY: updated[m].CY,CRT_ID: updated[m].CRT_ID,CRT_DT: updated[m].CRT_DT});
                    		}
                    	}
                    	//批量修改
                        var ajaxUpdate = {
...
//			                        	dataGrid.datagrid('deleteRow', indexs[j]);
	...
	nationalMaintenance.prototype = {
		init: function() {
			$(function() {
				//初始化全局变量对象
				dataGrid = $('#national_tab'),dataCompany=[],dataFactory=[],showmessage=$('#showMessageInfo'),editIndex = undefined,oldRow=undefined, reg=new RegExp("null","g");
				initGridData();
				//获取工厂类别下拉
				$('.add').click(function() {					
					insertDataGrid();
