...
function printPreviewCj(LFU,Str) {
	Install_InsertReport();//判断插件是否已经安装
    var Installed = Install_Detect();//判断插件是否已经安装
    
    CreateReport("Report");
    Report.LoadFromURL(LFU);
    Report.LoadDataFromXML(Str);//从XML或JSON文字串中载入报表明细记录集数据，数据应符合约定的形式
    
    //Report.PrintPreview(true);//全屏
    Report.PrintPreviewEx(4,true,false);//后面的false是不全屏的意思
    javascript:history.back(-1)//浏览器后退一页
}
