...
//				           { field : "CZ",width : 10,checkbox : true}, 
				           { field : 'SC_NMU',title : pageConfig.gcDictCD,width : 200,align : 'center'}, 
				           { field : 'SC_NAME',title : pageConfig.gcDictName,width : 200,align : 'center'},
				           { field : 'SC_RM',title : '备注',width : 200,align : 'center'}, 
		...
		saveUpdateValidate = function() {
			...
			if (row.SC_NMU) {
				...
				if ($('#'+pageConfig.txtDictName).val() != (row.SC_NAME==null?'':row.SC_NAME)
					|| $('#'+pageConfig.dictRemark).val() !=(row.SC_RM==null?'':row.SC_RM)
		...
		updateDictItem = function() {
			...
			if (row){
				...
				$('#' + pageConfig.txtDictCode).textbox('setValue',row.SC_NMU);
				$('#' + pageConfig.txtDictName).textbox('setValue',row.SC_NAME);
				$('#' + pageConfig.dictRemark).textbox('setValue',row.SC_RM);
				if (row.USE_YN == "Y") {
		    ...
		    $.messager.confirm('确认框','您确定要删除您所选择的数据?',
		      function(r){
		    	  if(r==true){
		    		 $.each(checkedItems,function(index,item){
		    			 delCnt++;
		    			 arrUpdate.push({SC_NMU:item.SC_NMU});
		    			 var ajaxParam = {
		    				 url : '/iPlant_ajax',
			    			 dataType : 'JSON',
			    			 data:{
		...
		saveDictItem = function() {
			...
			if ($("input[name='postingdate']").is(':checked'))
				 ...
				 isUsed = 'N';
			var reqData = {
					SC_NMU : $("#"+pageConfig.txtDictCode).val(), // 维修类别编号
					 SC_NAME : $("#"+pageConfig.txtDictName).val(), // 维修类别名称
					SC_RM : $('#' + pageConfig.dictRemark).val(), // 备注
					USE_YN : isUsed
			}
