	...
	function application() {		
		initGridData = function() {
			...
			bindGridData = function(reqData, jsonData) {
                ...
                return "<span title='" + value + "'>" + value + "</span>";}
					...
					onDblClickRow: function(index,row){	
				    	 ...
				    	 checkFun();
				    	    $('#txtWAREHOUSE_TYPE_ID').textbox('setValue', row.WAREHOUSE_TYPE_ID==null?'':row.WAREHOUSE_TYPE_ID);
							$('#txtWAREHOUSE_TYPE_NAME').textbox('setValue', row.WAREHOUSE_TYPE_NAME==null?'':row.WAREHOUSE_TYPE_NAME);
			...
			checkFun = function (){
				...
				if(qx=="Y"){
					 $('#txtWAREHOUSE_TYPE_ID').textbox('textbox').attr('disabled', true);
			    	 $('#txtWAREHOUSE_TYPE_NAME').textbox('textbox').attr('disabled', false);
...
//			    	 $('#cancleID').hide();
		...
		setDataNull = function () {           
              $('#txtWAREHOUSE_TYPE_ID').textbox('setValue','');              
          }
		getDataBySearch = function(){
				var dgrid = $('#warehouseType_tab').datagrid('options');
				if(!dgrid) return;
				var cxwarehouseDesType_id = $('#warehouseDesType_id').val();
				var cxwarehouseDesType_name = $('#warehouseDesType_name').val();				
				var reqData = {
		...
		updateStation = function() {
			...
			if(row) {				
				...
				$('#txtWAREHOUSE_TYPE_ID').textbox('textbox').attr('readonly', true);
				$('#txtWAREHOUSE_TYPE_ID').textbox('textbox').attr('disabled', true);
				$('#txtWAREHOUSE_TYPE_ID').textbox('setValue', row.WAREHOUSE_TYPE_ID==null?'':row.WAREHOUSE_TYPE_ID);
				$('#txtWAREHOUSE_TYPE_NAME').textbox('setValue', row.WAREHOUSE_TYPE_NAME==null?'':row.WAREHOUSE_TYPE_NAME);
				$('#txtENABLE').val(row.ENABLE);
        ...
        getApplyOrder =function(){
            ...
            var ajaxParam={
                ...
                successCallBack:function(data){
                    ...
                    $('#txtWAREHOUSE_TYPE_ID').textbox('setValue',workOrderData.WAREHOUSE_TYPE_ID).textbox('readonly').textbox({ required: true });                                                                         
                }
            } 
            iplantAjaxRequest(ajaxParam); 
        }
...
//        	getApplyOrder();
//        	checkFun();
//        	$('#txtWAREHOUSE_TYPE_ID').textbox('textbox').attr('readonly',false);
//			$('#txtWAREHOUSE_TYPE_ID').textbox('textbox').attr('disabled',false);
        ...
        saveUpdateValidate = function() {
			...
			if (row.WAREHOUSE_TYPE_ID) {
				...
				if ($('#txtWAREHOUSE_TYPE_ID').textbox('getValue') != (row.WAREHOUSE_TYPE_ID==null?'':row.WAREHOUSE_TYPE_ID)
						|| $('#txtWAREHOUSE_TYPE_NAME').textbox('getValue') != (row.WAREHOUSE_TYPE_NAME==null?'':row.WAREHOUSE_TYPE_NAME)						
						|| isUserYn != row.USE_YN
		...
		savaStation = function() {
			...
			var reqData = [];
		    if(CompanyOpttype == 0) {
				IFServerNo = 'WMS_B000016'
			} else if(CompanyOpttype == 1) {
				if (!saveUpdateValidate()) {
					$.messager.alert("提示", '内容没有更新，请修改')
					return false;
				}
				IFServerNo = 'WMS_B000015'
			} else {
				IFServerNo = 'WMS_B000014'
			}
		    var useYn = '';
			if ($('#txtENABLE').is(':checked')) {
				useYn = "Y";
			...
			var ajaxParam = {
				...
				dataType: 'JSON',
				data: {
					WAREHOUSE_TYPE_ID: $('#txtWAREHOUSE_TYPE_ID').val(),
					WAREHOUSE_TYPE_NAME: $('#txtWAREHOUSE_TYPE_NAME').val(),
					ENABLE: useYn,
  				    NOTE: $('#txtNOTE').val(),									
					IFS: IFServerNo
				},
				successCallBack: function(data) {
                	...
                	$.messager.alert("提示",susMsg,"",function(){
            			reqGridData('/iPlant_ajax','warehouseType_tab',{IFS:'WMS_B000014'});
