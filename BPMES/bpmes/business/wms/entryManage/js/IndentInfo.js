	 				...
	 				$('#TaryInfo_tab').datagrid({
	 	                ...
	 	                columns: [[
	 	                    ...
	 	                    {field:'DETAIL_ID', title: 'DETAIL_ID', width:80,align:'center',hidden:true},
	 	                   	{field:'MATERIA_ID',title: '物料信息<span style="color:red">*</span>', width:180,align:'center',
...
//	 	                                		}
//	 	                                	}
	 										var target = $('#TaryInfo_tab').datagrid('getEditor', {'index':ccIndex,'field':'ITEM_NM'}).target;
	 										target.textbox('setValue', wl[data.id]);
	 									}
	 	                            }    
	 	                        }
	 	                   },
	 	                   {field:'ITEM_NM', title: '物料名称<span style="color:red">*</span>', width:150,align:'center',editor:{type:'textbox',options:{editable:false}}},               
...
//    	}
    	...
    	for(var i=0;i<sendData.length;i++){
    		...
    		unitList[i]={
	    			  ...
	    			  MATERIA_NAME:sendData[i].ITEM_NM,
...
OpenImprotFramedr = function(){
	$("#enditTabupload").dialog("open").dialog('setTitle', '来料导入');
}	
...
//            array.push({ "id": "", "text": "全部"});
            for (var i = 0; i < data.RESPONSE[0].RESPONSE_DATA.length; i++) {
            	dataWL.push({ "id": data.RESPONSE[0].RESPONSE_DATA[i].ITEM_CD, "text": data.RESPONSE[0].RESPONSE_DATA[i].ITEM_CD });
