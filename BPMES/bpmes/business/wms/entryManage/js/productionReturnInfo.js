...
(function() {
	function TrayInfo2() {
				...
				initGridData = function() {
					...
					/** 货位信息 */
					/*
					 * var positionbox = { url: "/iPlant_ajax", dataType:
					 * "JSON", data: {IFS: "WMS_B00028"}, successCallBack:
					 * function(a) { var op = a.RESPONSE[0].RESPONSE_DATA;
					 * $.each(op,function(n,obj) {
					 * position.push({'value':obj.POSITION_ID,'text':obj.POSITION_NAME});
					 * }); }, errorCallBack: function() { $.messager.alert("提示",
					 * '请联系管理员，查询失败！') } }; iplantAjaxRequest(positionbox)
					 */
					// 供应商
					var suppiler = {
						url : "/iPlant_ajax",
...
//									'text' : obj.SUP_NM
							...
							for (var i = 0; i < rowCollection.length; i++) {
								datasupplier.push({
									"id" : rowCollection[i].SUP_CD,
									"text": rowCollection[i].SUP_NM
								});
								Idatasupplier[rowCollection[i].SUP_CD]=rowCollection[i].SUP_CD;
								arrList[rowCollection[i].SUP_CD]=rowCollection[i];
							};
						},
						errorCallBack : function() {
							$.messager.alert("提示", '请联系管理员，查询失败！')
									...
									editor : {
										...
										options : {
											...
											onSelect:function(record){
												var objGrid=$("#TaryInfo_tab");
												var bp = objGrid.datagrid('getEditor', {index:0,field:'SUPPLIER_NAME'});
												$(bp.target).textbox('setValue', arrList[record.id].SUP_NM);
...
//							var edditmpWarehouse = $(this).datagrid(
//									'getEditor', {
//										index : index,
//										field : 'WAREHOUSE_ID'
//									});
//							row.WAREHOUSE_ID = $(edditmpWarehouse.target)
//									.combobox('getValue');
//							row.WAREHOUSE_NAME = $(edditmpWarehouse.target)
//									.combobox('getText');
//							var edditmpStore = $(this).datagrid('getEditor', {
//								index : index,
//								field : 'STORE_ID'
//							});
//							row.STORE_ID = $(edditmpStore.target).combobox(
//									'getValue');
//							row.STORE_NAME = $(edditmpStore.target).combobox(
//									'getText');
//							var edditmpShelf = $(this).datagrid('getEditor', {
//								index : index,
//								field : 'SHELF_ID'
//							});
//							row.SHELF_ID = $(edditmpShelf.target).combobox(
//									'getValue');
//							row.SHELF_NAME = $(edditmpShelf.target).combobox(
//									'getText');
//						},
//						/** 进入编辑模式的操作 */
...
//				},
				...
				onEndEdit : function(index, row) {
					...
					row.POSITION_NUMBER = parseInt(row.COL_NUMBER)
							* parseInt(row.LAYER_NUMBER);
...
//									var rowEdit = dataGrid.datagrid('getRows')[editIndex], edft = $(
//											this).datagrid('getEditor', {
//										index : editIndex,
//										field : 'ORDER_ID'
//									}), editorFt = edft.target, ORDER_ID2 = editorFt
//											.val();
...
//				},
				...
				insertDataGrid = function() {
					var row = dataGrid.datagrid('getSelected'),compV='',compT='';
//					if (datasupplier.length > 0) {
//						compV = datasupplier[0].value,
//								compT = datasupplier[0].text;
//					}
...
//                            SUP_NM:compT
		...
		var ajaxParam = {
			...
			successCallBack : function(data) {
				...
				for (var i = 0; i < data.RESPONSE[0].RESPONSE_DATA.length; i++) {
					array.push({
						"id" : data.RESPONSE[0].RESPONSE_DATA[i].ITEM_CD,
						"text" : data.RESPONSE[0].RESPONSE_DATA[i].ITEM_NM
					});
				}
				console.log(array);
				$('#MATERIA_ID').combobox({
	...
	TrayInfo2.prototype = {
		init : function() {
			$(function() {
				//初始化全局变量对象
				dataGrid = $('#TaryInfo_tab'), warehouse = [], store = [],
						goodsShelf = [], datasupplier=[],Idatasupplier=[],arrList={},
