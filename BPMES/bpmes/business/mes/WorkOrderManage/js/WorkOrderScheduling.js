...
(function() {
	...
	function getDataBySearch(materialNoArray){
	    ...
	    var mono = $("#moNo").textbox('getValue');
    ...
    $(function(){
    	...
    	$("input",$("#txtProdCnt").next("span")).blur(function(){ 
		   var txtStationCode = $('#txtProdCnt').val();
		   if(txtStationCode!='' && txtStationCode>0){
   			 curInputQty=parseInt(txtStationCode);
   		   }else{
   			 $.messager.alert('提示', '排产数量不能为空且需大于零！');
    	...
    	$('#btnSetWorkOrder').click(function(){
    		 ...
    		 curInputQty=$('#txtProdCnt').textbox('getValue');
    		 ...
    		 */
    		 ...
    		 if(stationName.indexOf('注塑')>-1 )
    		 {
                ...
                /*排产数量的校验*/
        		 if(curInputQty>curPoRemainQty){
        			 $.messager.alert('提示', '排产数量应该小于或等于剩余数量,工单:'+curFouceMO+'剩余数量为:'+curPoRemainQty);
        			 return;
        		 }
             }
    		 ...
    		 if(curInputQty>curRemainQty){
    			 $.messager.alert('提示', '排产数量应该小于或等于剩余数量');
    			 return;
    		 }
