(function() {
	...
	function dictItem() {
		...
		initGridData = function(et,a,et1) {
		...
		bindGridData = function(reqData, jsonData) {
			var grid = {
				...
				columns : [[ 
				           ...
				           { field : 'TYPECODE',title : '类型编号',width : 200,align : 'center'},
				           { field : 'AS_NUM',title : '限制次数',width : 200,align : 'center'},
				           ...
				           {field : 'CRT_DT',title : '创建日期',width : 200,align : 'center'},
				           {field : 'RM',title : '备注',width : 200,align : 'center'},
				         ]]
			}
		...
		checks = function (checkedItems){
			$("#enditTab").dialog("open").dialog('setTitle', '<font color=\"white\">查看工装设备类型</font>');
    		$('#txtListCode').textbox('setValue',checkedItems[0].TYPECODE);
    		$('#txtListName').textbox('setValue',checkedItems[0].TYPENAME);
    		$('#remark').textbox('setValue',checkedItems[0].RM);
    		$('#useNumber').textbox('setValue',checkedItems[0].AS_NUM);//限制次数 
		...
		updateDictItem = function() {
			...
			if (row){
				...
				$('#' + pageConfig.txtDictCode).textbox('setValue',row.TYPECODE);//类型编号
				$('#' + pageConfig.txtDictName).textbox('setValue',row.TYPENAME);//类型名
				$('#remark').textbox('setValue',row.RM);//备注  
				$('#useNumber').textbox('setValue',row.AS_NUM);//限制次数 
				if (row.USE_YN == "Y") {
					$('#' + pageConfig.cbUsed).prop('checked','checked');
				} 
				else{
		    ...
		    deleteDictItem=function(){
			   ...
			   var FT_TYPES= new Array(); 
			   $.messager.confirm('确认框','您确定要删除您所选择的数据?',
			 function(r){
				   if(r==true){
					   $.each(checkedItems,function(index,item){
						   delCnt++;
			    			 arrUpdate.push({TYPECODE:item.TYPECODE});
			    			 FT_TYPES.push({FT_TYPE:item.TYPENAME});
			    			 var ajaxParam = {
			    				 url : '/iPlant_ajax',
				    			 dataType : 'JSON',
				    			 data:{
		...
		saveDictItem = function() {
			...
			if ($("input[name='postingdate']").is(':checked'))
				 ...
				 isUsed = 'N';
			var reqData = {
					TYPECODE : $("#" + pageConfig.txtDictCode).val(), // 类型编号
					TYPENAME:$("#" + pageConfig.txtDictName).val(), // 类型名称
					AS_NUM :$("#useNumber").val(),//限制次数
					RM : $("#remark").val(), // 备注
					USE_YN:isUsed
			}
