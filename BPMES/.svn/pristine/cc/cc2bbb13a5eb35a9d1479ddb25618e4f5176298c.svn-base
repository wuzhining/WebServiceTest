 ...
 */
(function() {
	function smtWorkFlow() {
		initGridData = function() {
			queryFlow();
		},
		bindGridData = function(reqData, jsonData) {
			var grid = {
				name : 'smtWorkFlow_tab',
				dataType : 'json',
			...
			$('#smtWorkFlow_tab').datagrid('loadData', jsonData);
		},
		
		queryFlow = function(){
			var dgrid = $('#smtWorkFlow_tab').datagrid('options');
			if (!dgrid)
				return;
			var reqData = {
				IFS : 'ACT0000',
			...
			reqGridData('/iPlant_ajax', 'smtWorkFlow_tab', reqData);
		},
