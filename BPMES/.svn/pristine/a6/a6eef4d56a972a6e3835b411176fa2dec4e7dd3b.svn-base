		...
		bindGridData = function(reqData, jsonData) {
			var gridList = {
				...
				columns: [[				
			        ...
			        {field: 'PLAN_METHOD',title: '排程方法',width: 100,align: 'center',formatter: function (value,row) {if(value == '1') { return '拉式排程';} else { return '推式排程';}/*if(checkNotEmpty(value)) return "<span title='" +  value + "'>" +  (row.PLAN_METHOD || value)+ "</span>";*/},
					    	   editor:{type:'combobox',options:{valueField:'value',textField:'text',data:[{'value':'1'	,'text':'拉式排程'},{'value':'2','text':'推式排程'}],required:true,editable:false}}}, 
	               ...
	               {field: 'PROD_CON',title: '产能约束',width: 100,align: 'center',formatter: function (value,row) {if(value == '1') { return '有限能力';} else { return '无限能力';}/*if(checkNotEmpty(value)) return "<span title='" +  value + "'>" +  (row.PROD_CON || value)+ "</span>";*/},
				    	editor:{type:'combobox',options:{valueField:'value',textField:'text',data:[{'value':'1','text':'有限能力'},{'value':'2','text':'无限能力'}],required:true,editable:false}}}, 
	...
	factoryInfo.prototype = {
		init: function() {
			$(function() {
				...
				/*排程方法*/
				var PLAN_METHOD = [{'value':'1','text':'推式排程'},{'value':'2','text':'拉式排程'}];
				...
				/*产能约束*/
				var Prod_Con = [{'value':'1','text':'有限能力'},{'value':'2','text':'无限能力'}];
				$('#Prod_Con').combobox({
                    data:Prod_Con,
                    valueField:'value',
                    textField:'text',
				...
				$('#btnAdd').click(function() {
					...
					var Prod_ConValue = $('#Prod_Con').combobox('getValue');
					...
					if(WC_CD.length>0 && PRF_CD.length>0){
						initData={WC_CD:WC_CDValue,PLAN_CYCLE:PLAN_CYCLE,PLAN_METHOD:PLAN_METHODValue,PRF_CD:PRF_CDValue,
								LINE_CD:LINE_CDValue,SHIFT_CD:SHIFT_CDValue,MAT_YN:MAT_YNValue,Prod_Con:Prod_ConValue,CAPA_YN:CAPA_YNValue
						}
					}
