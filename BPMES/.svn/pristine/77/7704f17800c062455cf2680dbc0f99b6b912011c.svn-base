...
(function() {
	function materialMaintenance() {
		initGridData = function() {
			QueryProcess();
			var dgrid = $("#MaintenanceMethod_tab").datagrid('options');
			if(!dgrid) return;
			var pi = {
	                url: "/iPlant_ajax",
	                dataType: "JSON",
		...
		bindGridData = function(reqData, jsonData) {
			var gridList = {
				...
				columns: [[  
					        ...
					        {field: 'MT_FUN_CD',title: '维修方法代码',width: 100,align: 'center',formatter: function (value) {if(checkNotEmpty(value)) return "<span title='" + value + "'>" + value + "</span>";},editor:{type:'validatebox',
						           options:{required:true, validType:['length[1,30]','specialTextCharacter']}}},
							{field: 'MT_FUN_NM',title: '维修方法描述',width: 150,align: 'center',formatter: function (value) {if(checkNotEmpty(value)) return "<span title='" + value + "'>" + value + "</span>";},editor:{type:'validatebox',
					        	   options:{required:true, validType:['length[1,30]','specialTextCharacter']}}},
					        {field: 'MT_FUN_ST',title: '状态',width: 100,align: 'center',formatter: function (value,row) {if(checkNotEmpty(value)) return "<span title='" +  value + "'>" +  (row.MT_FUN_ST || value)+ "</span>";},
								   editor:{type:'combobox',id:"status",options:{valueField:'value',textField:'text',data:dataFactory,required:true}}},
		        	...
		        	/**判断是否为可编辑字段*/
			        	if (editIndex != index){
				    		...
				    		if(editIndex!=undefined){
				    			...
				    			rowEdit = dataGrid.datagrid('getRows')[editIndex],editem = $(this).datagrid('getEditor', {index: editIndex,field: 'MT_FUN_CD'}),editorFt = editem.target,itemCd = editorFt.val();
				    			if(checkNotEmpty(rowEdit.editType)){
				    				if(rowEdit.editType=='add'){
				    					...
				    					if(!checkNotEmpty(row.editType)){			/*如果是修改的情况，ft_cd字段为只读模式*/
								    		ed = $(this).datagrid('getEditor', {index: index,field: 'MT_FUN_CD'});
								    		fc = ed.target;
		...
		searchDataGrid=function(){
			var dgrid=$("#MaintenanceMethod_tab").datagrid("options"),MT_FUN_NM = $('#MT_FUN_NM').textbox('getValue'),PRNT_CD = $('#PRNT_CD').combobox('getValue');
			var reqData = {
				IFS: 'E000009',
				MT_FUN_NM:MT_FUN_NM,
				PRNT_CD:PRNT_CD,
			...
			reqGridData('/iPlant_ajax', 'MaintenanceMethod_tab', reqData);
		},
		...
		savaStationA = function(){
			...
			var work = {
                    url: "/iPlant_ajax",
                    dataType: "JSON",
                    data: {IFS: "E0000017",PRNT_CD:PRNT_CD},
                    successCallBack: function(a) {
                    	if(a.RESPONSE[0].RESPONSE_DATA.length <= 0){
                			...
                			iplantAjaxRequest(ajaxParamTimeAA);
                    	}else{
                    		$("#enditTab").dialog("close");
		...
		init: function() {
			$(function() {
				var ajaxParam1={
		                ...
		                successCallBack:function(data){
		                	$('#PRNT_CD').combobox('clear');
		                    var arr = [];
		                    var op = data.RESPONSE[0].RESPONSE_DATA;
		                    arr.push({"value":"", "text":"全部"});
		                    for(var i=0; i< op.length; i++){
		                    	arr.push({"value":op[i].sT_C_CD, "text":op[i].sT_C_NM});
		                    }
		                    $('#PRNT_CD').combobox({
		                        data:arr,
		                        valueField:'value',
				...
				/*初始化全局变量对象*/
				dataGrid = $('#MaintenanceMethod_tab'),dataCompany=[],dataTmp=[],dataPi=[],dataFactory=[],showmessage=$('#showMessageInfo'),editIndex = undefined,oldRow=undefined, reg=new RegExp("null","g");
				bindTreeData();
				initLeftMenu();
				initGridData();
				...
				$('#btnAdd').click(function() {					
					insertDataGrid('MaintenanceMethod_tab',{});
				});
				
				$('#btnDelete').click(function(){
					deleteDataGrid('MaintenanceMethod_tab','MT_FUN_CD','E0000012','showMessageInfo');
					initGridData();
					initLeftMenu();
					bindTreeData();
	            });
				...
				$('#btnSave').click(function() {
					saveDataGrid('MaintenanceMethod_tab','E0000010','E0000011','showMessageInfo');
					initGridData();
					initLeftMenu();
					bindTreeData();
				});
				...
				$('#btnExprt').click(function(){
                	...
                	createTable('tbIMESReport','维修方法导出','MaintenanceMethod_tab',reqData);
                });
