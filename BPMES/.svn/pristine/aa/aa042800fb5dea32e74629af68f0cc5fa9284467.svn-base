...
(function() {
	function factoryInfo() {
		...
		saveMesSNcodes = function(){
			var workOrdeSNs = $('#turnoverBoxBarCodePrintQuerybottom_tab').datagrid('getSelections');	/*获取所有选择的工单条码*/
			var data1=new Array();
			var barCodeList="";
			/*获取当前日期作为打印日期*/
			function p(s) {
		...
		openPrintPreview = function(){
			...
			var grid = $('#turnoverBoxBarCodePrintQuerybottom_tab').datagrid('getSelected');
			
		
			$("#txtNOW_PRINT_TIME").textbox('setValue',now);
			$("#txtBOX_CD").textbox('setValue',grid.BOX_CD);
			$("#txtBOX_BARCODE").textbox('setValue',grid.BOX_BARCODE);
