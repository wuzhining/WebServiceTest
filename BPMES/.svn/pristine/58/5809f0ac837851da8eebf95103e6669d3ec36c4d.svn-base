...
//				           { field : "CZ",width : 10,checkbox : true}, 
				           ...
				           { field : 'MT_FLAG',title : '备注',width : 200,align : 'center'}, 
				           ...
				           { field : 'UPT_ID_NM',title : '修改人',width : 200,align : 'center'}, 
		...
		saveUpdateValidate = function() {
			...
			if (row.DICT_CD) {
				...
				if ($('#'+pageConfig.txtDictName).val() != (row.DICT_NM==null?'':row.DICT_NM)
					|| $('#'+pageConfig.dictRemark).val() !=(row.MT_FLAG==null?'':row.MT_FLAG)
		...
		updateDictItem = function() {
			...
			if (row){
				...
				$('#' + pageConfig.dictRemark).textbox('setValue',row.MT_FLAG);
				if (row.USE_YN == "Y") {
		...
		saveDictItem = function() {
			...
			else if($("#" + pageConfig.txtDictCode).val() == null || $("#" + pageConfig.txtDictCode).val() == ""){
			...
			var reqData = {
				DICT_IT : count++, // 字典代码
				DICT_NM :  $("#"+pageConfig.txtDictName).val(),// 设备类别名称
				DICT_CD : $("#"+pageConfig.txtDictCode).val(), // 设备类别编号
				MT_FLAG : $('#' + pageConfig.dictRemark).val(), // 备注
				USE_YN : isUsed
			}
