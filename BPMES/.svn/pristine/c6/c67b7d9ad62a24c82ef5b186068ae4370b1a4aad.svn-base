...
public class ZplPrinter extends BaseBusiness{  
	...
	public CommonResult execute(DataExchangeAssembly dataExchange) {
		...
		//判断是否是当天的时间
		String barCode = (String) parameter.get("barCode");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String today = sdf.format(new Date());
		String tempDay = (String) parameter.get("content");
		
		int returnNum = 0;
		if(barCode.equals("0")){}else if(barCode.equals("1")){
	...
	public String printer(String barCode) {  
        ZplPrinter p = new ZplPrinter("\\\\192.168.50.112\\ZDesigner ZT410-300dpi ZPL");  
        //1.打印单个条码  
    ...
    public ZplPrinter(String printerURI){  
        ...
        //获取配置文件加载字体 
        File file = new File("D://project//sysUI//Trunk//IplantUI//upload//printer//ts24.lib");  
        if(file.exists()){  
            FileInputStream fis;  
            try {  
