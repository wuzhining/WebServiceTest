...
(function(){
	function csbkbInfo(){
		ccfunc = function (){
	        ...
	        /*五天内设备产能*/
			screenAjaxFun('MES_R0109',lx,dtime,sbbm).then(function(rowCollection){
				var tm = [];
	        	var Aarr = [];
				if(rowCollection.length>0){
					for(var i=0; i<rowCollection.length; i++){
     				   tm.push(rowCollection[i].TM.substring(5,rowCollection[i].TM.length));
	        			   Aarr.push(parseInt(rowCollection[i].PRODUCTION_COUNT));
	        		};
     		    };
     		    barChart('container1','设备产能',Aarr,'完成数',tm);
			});
