...
(function() {
	function logicFactory() {
		...
		initGridData = function() {
			...
			reqGridData('/iPlant_ajax', 'logicFactory_tab', reqData);
		},
		bindGridData = function(reqData, jsonData) {
			var gridList = {
				name: 'logicFactory_tab',
				dataType: 'json',
				columns: [[
				    {field: 'CD',title: '编号',width: 100,align: 'center',hidden:'true'},
					{field: 'LG_FT_CD',title: '逻辑工厂代码',width: 100,align: 'center',formatter: function (value) {if(checkNotEmpty(value)) return "<span title='" + value + "'>" + value + "</span>";},editor:{type:'validatebox',
...
//			                        	dataGrid.datagrid('deleteRow', indexs[j]);
	...
	logicFactory.prototype = {
		init: function() {
			$(function() {
				//初始化全局变量对象
				dataGrid = $('#logicFactory_tab'),dataCompany=[],dataFactory=[],showmessage=$('#showMessageInfo'),editIndex = undefined,oldRow=undefined, reg=new RegExp("null","g");
				initGridData();
				//获取工厂类别下拉
				$('.add').click(function() {					
					insertDataGrid();
